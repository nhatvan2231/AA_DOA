...
for dir in ~/AA_DOA/src/data/rss360_noise/*.dat; do
	echo "\n${dir}"
	...
	#cat ~/AA_DOA/src/tools/signal_1khz.bin | ~/ncpa/libNCPA/bin/binary2human --type double --N_channels 1 | head -n -1 | ./noisePropagator --arrayGeometry AA_geometry.tsv --signalInfo "${dir}" --dataSample 100 > "$OUTPUT"
done
